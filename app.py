    ...
    if submitted:
        ...
        try:
            ...
            if not st.session_state.fraud_models:
                ...
                # Get the best model name from session state
                best_model_name = st.session_state.best_fraud_model
            ...
            with col2:
                ...
                st.markdown(f"**Confidence Level:** {'High' if max(fraud_prob, 1-fraud_prob) > 0.8 else 'Medium' if max(fraud_prob, 1-fraud_prob) > 0.6 else 'Low'}")
                st.markdown(f"**Transaction Status:** {'⚠️ Requires Review' if fraud_prob >= 0.4 else '✅ Normal Processing'}")
            ...
            if explanation:
                ...
                for factor in explanation[:5]:  # Show top 5 factors
                    st.markdown(f"• {factor}")
            
            # Feature importance visualization
            try:
                if 'fraud_models' in st.session_state and best_model_name in st.session_state.fraud_models:
                    model_info = st.session_state.fraud_models[best_model_name]
